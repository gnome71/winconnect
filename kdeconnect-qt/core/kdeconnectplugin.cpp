...
bool KdeConnectPlugin::sendPackage(NetworkPackage& np) const
{
    if(!d->mOutgoingCapabilties.contains(np.type())) {
		qWarning() << metaObject()->className() << "tried to send an unsupported package type" << np.type() << ". Supported:" << d->mOutgoingCapabilties;
        return false;
    }
	 qWarning() << metaObject()->className() << "sends" << np.type() << ". Supported:" << d->mOutgoingCapabilties;
